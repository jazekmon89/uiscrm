...
@foreach($current->questions as $question)
	@php
		...
		if($question->FormQuestionTypeName){
			if(in_array($question->Name,['FullTimeStaffCount','PartTimeStaffCount']))
				$label_column_width_class = 'col-md-6 m-right-9';
			elseif($question->Name == 'Address' || $current->Name == 'SituationAtRisk')
				...
				$label_column_width_class = 'col-md-4';			
	...
	<div class="{{ implode(' ', $cls) }}" 
		...
		<div class="form-group {{ $question->FormQuestionTypeName == 'Address'?'col-md-9':$column_width_class }} btn-group-vertical">
			...
			@elseif($question->FormQuestionTypeName =='SelectOne')
				...
				@else
					{{ Form::jInput("select", "{$baseKey}.$question->FormQuestionID", ["" => ""] + $choices, null, ['class' => 'form-control' ])}}
				@endif
