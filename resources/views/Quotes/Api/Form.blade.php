...
@foreach($groups as $group)
	...
	@endpush
	@push('form-content') 
		<div class="rfq-group hidden {{ $loop->last ? 'last' : '' }}" id="tab-{{ $group->Name }}" >
			{{Form::open(['route' => [$rname .'quotes.validate', $PolicyTypeID, $FormTypeID, $group->FormQuestionGroupID, 'OrganisationID' => $OrganisationID, '_token' => csrf_token()], 'class' => 'grid-60 centered'])}}

				<div class="form-group grid-100 disp-block button-group text-right">
						@if($loop->first)
				...
				<div class="grid-100 disp-inline-table">
					<h4 class="form-tab-header grid-70 disp-inline-block form-section-title v-space-5x">{{ $group->DisplayText }}</h4>
				</div>
				...
				<div class="form-footer v-space-4x grid-100 disp-block">
