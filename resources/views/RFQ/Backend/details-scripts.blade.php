	<script>
		...
		function reCreateRFQ(UserID, callback)
		{
			...
			$.ajax({
				...
				url: '{{ route('rfqs.re-create', [$RFQ['RFQID'], 'CONTACTUSERID']) }}'.replace('CONTACTUSERID', UserID),
				success: function(result) 
				{
					
					if (result.success)
					{
						location.href = '{{ route('rfqs.view', 'RFQID') }}'.replace('RFQID', result.RFQID);
					}
					else
					{
						for(var i in result)
		...
		function non_proceed()
		{
			if (confirm("Are you sure to set this RFQ as non proceed ?"))
			{
				//update status
				$.ajax({
					url: '{{ route('rfqs.non-proceed', $RFQID) }}',
					type: 'post',
		...
		$(document).ready(function() {
		   ...
		   $('form.updateExpiryDate').submit(function(e) {
		   		e.preventDefault();

		   		var new_expiry_date = $(this).find('#ExpiryDate').val();

		   		var conf = "{{ trans('messages.confirmation_expiry_date') }}".replace(":date", new_expiry_date);
		   		...
		   		if(confirm(conf)) { 
		   			...
		   			$.ajax({
			        url: "{{ route('rfqs.updateExpiryDate') }}",
			        type: "POST",
			        data: {
			        		ExpiryDate :new_expiry_date, 
			        		RFQID : $(this).find('#RFQID').val(),
			        		_token : '{{ csrf_token() }}'
			        },
			        success: function(data){
			          if(data == 'false')
			            alert('Failed to updated expiry date.');
			          else{
